...
PROJECTS = [
    {
        ...
        "point": "The alert threshold comes from putting a cost on each type "
                 "of error instead of maximising a score. Adding "
                 "<code>scale_pos_weight</code> to handle the imbalance made "
                 "the ranking worse.",
        "figure": figures.fraud,
        "caption": "Ranking quality and money kept. They're on different "
    ...
    },
    {
        ...
        "point": "Every lag is derived from the forecast horizon, so "
                 "<code>lag_1</code> can't be built at a 28-day horizon. "
                 "Getting this wrong leaks the future into the features.",
        "figure": figures.demand,
        "caption": "Error by model across six rolling origins, and error by "
    ...
    },
    {
        ...
        "point": "The split protocol moves the score more than the model "
                 "does, and changes which model you'd pick. A random pair "
                 "split leaks questions between train and test.",
        "figure": figures.semantic,
        "caption": "The same four models under both split protocols. The gap "
                   "between the two bars in each pair is the effect of the "
                   "protocol.",
    },
    {
        ...
        "point": "Implicit ALS written from scratch. If you pick a model on "
                 "accuracy alone, you get one that recommends the same "
...
def build(root: Path, out_path: Path) -> Path:
  ...
  <p class="lede">Four small machine learning projects. Each runs on a laptop
  CPU in under three minutes and has a test suite, and each is built around a
  decision someone would have to make instead of a leaderboard score.</p>
</header>""")
  ...
  in this repository, so the feature engineering, leakage tests and evaluation
  can be read and rerun. None of these numbers say anything about a real
  business.
</div>""")
  ...
  <code>python -m dashboard.build</code>. The figures are drawn with matplotlib
  from the same files, so they always match the tables.
  Source: <a href="https://github.com/Wrlog/ml-practice">Wrlog/ml-practice</a>.
</footer>""")
